...
**推荐可视化：** 并发原语 → State Machine + Comparison HTML；接口/类型 → ASCII 图；GC → Flow/Pipeline HTML
...
**推荐可视化：** 排序/DP → Algorithm Trace HTML；树/图 → Data Structure Ops HTML；双指针 → Algorithm Trace HTML
...
**推荐可视化：** 架构 → Architecture HTML；微服务/分布式 → Architecture + Timeline HTML
...
**推荐可视化：** Transformer → Flow/Pipeline HTML；注意力矩阵 → Data Structure Ops HTML；RAG 流程 → Architecture HTML
